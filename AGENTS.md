...
- 左侧导航按 **本体模型（本体建模 / 本体技能）· 本体实例（图谱 / 对象 / 关系）· 动力模型（动作 / 规则）** 分组（2026-09-18 更新）：本体列表已并入「总览」下方，当前本体状态在上方；总览「查看本体」进入本体建模。图谱里的类型视图仍归实例侧。平台分组只有数据资源与设置，图引擎连接在「设置 → 图引擎配置」管理。
- 全局版本操作放页头右侧，不另占一整行：始终显示运行版本（未发布时显示「尚未发布」），有草稿时再并列显示草稿版本；「创建草稿」「版本记录」及历史版本激活入口仍可用。
