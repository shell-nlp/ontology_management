...
- **对象页**：已发布视图下选了具体对象类型后走对象服务，工具栏可切「来源：自动 / 已物化 / 业务库」
  （`origin` 透传），表头显示"共 N 条"；草稿视图仍读快照（正在编辑的那一版）。
...
| S2 | 对象服务：类型 + 主键 → 对象 | **已于 2026-09-19 完成（含界面与 AI 接入）**：`@/lib/object-service`（门面 + 数据资源来源）+ `GET /api/objects`；对象页可选来源、图谱页可按类型从数据源加载、动作页主对象走对象服务并可"取进草稿"，AI 两个实例工具也已开放。**未做**：关系类型的数据来源（D2）——数据源来的节点在图上没有连线 | 只差关系的数据来源；见 D2 |
