...
- **接口投影**（2026-09-19）：本体建模画布中，无数据源的对象类型可以「提取为接口」。平台保留原对象类型与底层关系作为兼容影子，新增接口记录 `promotedFromEntityTypeId`，并把原关系转成接口关系约束；**不根据关系另一端自动推断 `implements`**。画布隐藏影子节点、显示紫色接口节点与紫色虚线实现边；接口承接的关系约束默认收起，点击接口/实现方或打开「接口连接」后才显示青绿色虚线关系。不要把这条 UI 投影误认为实例层数据被删除。
