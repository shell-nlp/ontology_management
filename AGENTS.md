...
- 左侧导航按 **本体模型（本体建模 / 本体技能）· 本体实例（图谱 / 对象 / 关系）· 动力模型（动作 / 规则）** 分组（2026-09-18 更新）：本体列表已并入「总览」下方，当前本体状态在上方；总览「查看本体」进入本体建模。图谱只保留实例视图，不再有「查看本体」切换；类型定义和关系类型在本体建模画布查看、搜索。平台分组只有数据资源与设置，图引擎连接在「设置 → 图引擎配置」管理。
