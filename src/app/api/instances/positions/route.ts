...
import { updateNodePositions } from "@/lib/instances";
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const updated = await updateNodePositions(target, input.items);
    await writeAuditEntry({ actorId: user.id, targetId: target.id, action: "POSITIONS_UPDATED", details: { count: input.items.length } });
