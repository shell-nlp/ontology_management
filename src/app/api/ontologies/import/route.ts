...
export async function POST(request: NextRequest) {
  try {
    ...
    const bundle = typeof input.bundle === "string" ? readOntologyBundle(JSON.parse(input.bundle)) : readOntologyBundle(input.bundle);
    ...
    return NextResponse.json({ ontology, versionId, entityCount, relationshipCount, warnings: plan.warnings }, { status: 201 });
  } catch (error) {
