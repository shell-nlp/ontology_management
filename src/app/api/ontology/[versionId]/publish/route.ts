...
import { applyStrongRules, ontologyDefinitionSchema, validateOntology } from "@/lib/ontology";
...
export async function POST(_: Request, context: { params: Promise<{ versionId: string }> }) {
  try {
    ...
    await writeAuditEntry({ actorId: user.id, targetId: row.target_id, action: "ONTOLOGY_PUBLISHED", details: { versionId: row.id } });
    return NextResponse.json({ published: true, versionId: row.id });
  } catch (error) {
