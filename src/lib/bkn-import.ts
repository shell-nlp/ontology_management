...
export function fromBknKnowledgeNetwork(raw: unknown): BknConversion {
  ...
  const entityTypes: OntologyDefinition["entityTypes"] = objectTypes.map((item) => {
    ...
    if (!resourceName) {
      ...
      return { id: newId, name, description: text(item.comment), displayProperty: displayKey, parents: [], properties, sources: [] } as OntologyDefinition["entityTypes"][number];
    }
  ...
  const mappings: OntologyDefinition["relationshipTypes"] = relationTypes.map((item) => {
    ...
    return { id: crypto.randomUUID(), name, description: text(item.comment), sourceEntityTypeId: from, targetEntityTypeId: to, properties: [] } as OntologyDefinition["relationshipTypes"][number];
  });
  ...
  const bundle = buildOntologyBundle({
    ...
    definition: { entityTypes, relationshipTypes: mappings, actionTypes: [], rules: [] },
  });
