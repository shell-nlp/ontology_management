...
export function GraphKindChoice({ value, onChange }: { value: GraphTargetKind; onChange: (kind: GraphTargetKind) => void }) {
  ...
  const searchable = CREATABLE_GRAPH_TARGET_KINDS.length + PLANNED_GRAPH_TARGETS.length > 6;
  const needle = filter.trim().toLowerCase();
  const supported = useMemo(() => CREATABLE_GRAPH_TARGET_KINDS.filter((item) => !needle || `${item.label} ${item.description} ${item.queryLanguageLabel}`.toLowerCase().includes(needle)), [needle]);
  ...
  return <div className="kind-picker" ref={bodyRef} onKeyDown={moveFocus}>
    ...
    <div className="kind-picker-body">
      <nav className="kind-picker-rail" aria-label="类型分类">
        <button type="button" className={category === "supported" ? "active" : ""} onClick={() => setCategory("supported")}>可新建<span>{CREATABLE_GRAPH_TARGET_KINDS.length}</span></button>
