...
import { CircleDot, Link2, LocateFixed, Network, Pencil, Plus, Search, Trash2, X } from "lucide-react";
...
export function GraphCanvas({
  ...
  const nodes = useMemo<Node[]>(() => graph.nodes.map((node, index) => {
    ...
    return {
      id: node.id,
      position: moved[node.id] ?? storedPosition(node) ?? { x: 390 + Math.cos(angle) * radius, y: 275 + Math.sin(angle) * radius },
  ...
  }), [editTarget, graph.nodes, moved, visibleNodeIds]);
