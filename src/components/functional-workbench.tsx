...
type View = "ontologies" | "overview" | "ontology" | "actions" | "rules" | "qa" | "mcp" | "skills" | "graph" | "entities" | "relations" | "targets" | "data" | "settings";
...
const NAV_SECTIONS: { label: string; items: readonly NavItem[] }[] = [
  ...
  { label: "语义模型", items: [["ontology", "本体草稿", BookOpen], ["skills", "本体技能", Sparkles]] },
...
function TargetManager({ targets, refresh, selectedId, onSelect, onNew, notify, fail }: { targets: Target[]; refresh: () => Promise<void>; selectedId: string; onSelect: (id: string) => void; onNew: () => void; notify: (text: string) => void; fail: (reason: unknown) => void }) {
  ...
  return <section className="stack">
    <div className="panel functional-panel target-action-bar"><div><span className="eyebrow">本体存储</span><b>{targets.length} 个可用存储资源</b><p className="subtle">{census ? `按存储后端分组：${census}。` : "正在加载存储资源。"}内置类型图由平台自带；Jena 连接可按需添加。</p></div><button className="action primary" onClick={onNew}><Plus size={15} />新建 Jena 连接</button></div>
