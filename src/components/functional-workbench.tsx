...
function Overview({ target, draft, published, runtimeTypes, onNavigate, onOpenOntology }: { target: Target | null; draft: Version | null; published: Version | null; runtimeTypes: RuntimeTypeSet | null; onNavigate: (view: View) => void; onOpenOntology: () => void }) {
  ...
  return <section className="panel functional-panel overview-panel"><span className="eyebrow">本体控制室</span><h2>{target ? "本体与运行时状态" : "开始登记第一个本体存储"}</h2>{target ? <><div className="overview-stats"><div className="overview-stat"><b>{entityTypes}</b><span>类</span><small>数据库中的标签</small></div><div className="overview-stat"><b>{entities}</b><span>对象</span><small>数据库中的节点</small></div><div className="overview-stat"><b>{relationshipTypes}</b><span>关系类型</span><small>数据库中的关系类型</small></div><div className="overview-stat"><b>{relationships}</b><span>关系</span><small>数据库中的关系</small></div></div><div className="overview-status"><span>草稿：<b>{draft ? `v${draft.version_number}` : "无"}</b></span><span>已发布：<b>{published ? `v${published.version_number}` : "无"}</b></span></div>{hasCensus && (entityTotal + relationTotal > 0 ? <div className="overview-census"><section className="census-panel entity"><div className="census-head"><CircleDot size={15} /><span>类分布</span><b>{entityRows.length}</b></div>{censusRows(entityRows, entityTotal, "entity")}</section><section className="census-panel relation"><div className="census-head"><Link2 size={15} /><span>关系类型分布</span><b>{relationRows.length}</b></div>{censusRows(relationRows, relationTotal, "relation")}</section></div> : <div className="overview-census-empty">图数据库中还没有数据。在「图谱」页创建节点与关系后，这里会展示每种类与关系类型的数量分布。</div>)}</> : <p>先在「本体存储」登记图数据库的连接信息，凭据会加密保存。</p>}<div className="functional-actions">{target && <button className="action" onClick={() => onNavigate("graph")}><Network size={16} />打开图谱管理</button>}{target && <button className="action" onClick={() => onNavigate("ontology")}><BookOpen size={16} />配置本体草稿</button>}<button className="action primary" onClick={() => target ? onOpenOntology() : onNavigate("targets")}><span className="arrow">→</span>{target ? "查看本体" : "登记本体存储"}</button></div></section>;
}
...
function OntologyManager({ definition, draft, targetId, user, runtimeTypes, refreshRuntimeTypes, save, validate, publish, notify, onOpenActions, fail }: { definition: Definition; draft: Version | null; targetId?: string; user: User; runtimeTypes: RuntimeTypeSet | null; refreshRuntimeTypes: () => void; save: (definition: Definition) => Promise<void>; validate: () => Promise<void>; publish: () => Promise<void>; notify: (text: string) => void; onOpenActions: () => void; fail: (reason: unknown) => void }) {
  ...
  const createEntity = async (id: string, payload: EntityPayload) => { try { const trimmed = payload.name.trim(); if (!trimmed) throw new Error("请填写类的名称。"); if (definition.entityTypes.some((item) => item.name === trimmed)) throw new Error("类的名称已存在。"); await save({ ...definition, entityTypes: [...definition.entityTypes, { id, name: trimmed, description: payload.description.trim(), displayProperty: payload.displayProperty.trim(), properties: payload.properties, source: payload.source }] }); notify("类已加入草稿。"); } catch (reason) { fail(reason); } };
  const createRelation = async (id: string, payload: RelationPayload) => { try { const trimmed = payload.name.trim(); if (!trimmed) throw new Error("请填写关系类型名称。"); if (!payload.sourceEntityTypeId || !payload.targetEntityTypeId) throw new Error("请选择关系的起始类与终止类。"); if (definition.relationshipTypes.some((item) => item.name === trimmed)) throw new Error("关系类型名称已存在。"); await save({ ...definition, relationshipTypes: [...definition.relationshipTypes, { id, name: trimmed, sourceEntityTypeId: payload.sourceEntityTypeId, targetEntityTypeId: payload.targetEntityTypeId, properties: payload.properties }] }); notify("关系类型已加入草稿。"); } catch (reason) { fail(reason); } };
  ...
  const addEntity = async (event: FormEvent) => { event.preventDefault(); try { if (!name.trim()) return; if (definition.entityTypes.some((item) => item.name === name.trim())) throw new Error("类的名称已存在。"); await save({ ...definition, entityTypes: [...definition.entityTypes, { id: crypto.randomUUID(), name: name.trim(), description: description.trim(), properties: [] }] }); setName(""); setDescription(""); } catch (reason) { fail(reason); } };
  ...
  const updateEntity = async (id: string, patch: { name: string; description: string; displayProperty?: string; properties: Property[]; source?: EntityType["source"] }) => { try { const next = structuredClone(definition); const idx = next.entityTypes.findIndex((item) => item.id === id); if (idx < 0) return; if (next.entityTypes.some((item, i) => i !== idx && item.name === patch.name.trim())) throw new Error("类的名称已存在。"); next.entityTypes[idx] = { ...next.entityTypes[idx], name: patch.name.trim(), description: patch.description.trim(), displayProperty: (patch.displayProperty ?? "").trim(), properties: patch.properties, source: patch.source }; await save(next); notify("类及其属性已更新。"); } catch (reason) { fail(reason); } };
  ...
  const deleteEntity = async (id: string) => { const item = definition.entityTypes.find((entry) => entry.id === id); if (!item) return; const affectedRelations = definition.relationshipTypes.filter((entry) => entry.sourceEntityTypeId === id || entry.targetEntityTypeId === id); if (!window.confirm(`删除类「${item.name}」？${affectedRelations.length ? `引用该类型的 ${affectedRelations.length} 个关系类型（${affectedRelations.map((entry) => entry.name).join("、")}）将一并删除。` : "该类型的属性定义将一并删除。"}`)) return; try { const next = structuredClone(definition); next.entityTypes = next.entityTypes.filter((entry) => entry.id !== id); next.relationshipTypes = next.relationshipTypes.filter((entry) => entry.sourceEntityTypeId !== id && entry.targetEntityTypeId !== id); await save(next); notify("类已从草稿中移除。"); } catch (reason) { fail(reason); } };
...
function GraphManager({ target, user, version, draft, runtimeTypes, mode, onModeChange, onSnapshotChange, notify, fail }: { target: Target | null; user: User; version: Version | null; draft: Version | null; runtimeTypes: RuntimeTypeSet | null; mode: "instances" | "ontology"; onModeChange: (mode: "instances" | "ontology") => void; onSnapshotChange: () => Promise<void>; notify: (text: string) => void; fail: (reason: unknown) => void }) {
  ...
  return <section className="stack">
    ...
    {mode === "instances" ? <>
      ...
      {ontologyGraph ? <GraphCanvas graph={ontologyGraph} targetId={target?.id} user={user} editable={false} definition={version?.definition ?? null} viewMode="ontology" notify={notify} fail={fail} /> : <div className="graph-empty"><Network size={27} /><b>正在读取本体骨架</b><span>将从图数据库加载实际存在的类和关系类型。</span></div>}
    </>}
  </section>;
}

/**
 * 对象详情里的动作入口：列出定义在这个对象所属类上的动作。
...
function ScopedActions({ definition, versionId, subjectId, labels, disabled, onRun }: { definition: Definition | null; versionId?: string; subjectId: string; labels: string[]; disabled: boolean; onRun: (actionId: string) => void }) {
  ...
  // 还没建草稿/发布过时没有版本可判，先按类展示全部动作，不假装知道可见性。
  useEffect(() => {
...
function EntityManager({ target, user, version, draft, runtimeTypes, ensureDraft, onSnapshotChange, notify, onRunAction, fail, entityLimit }: { target: Target | null; user: User; version: Version | null; draft: Version | null; runtimeTypes: RuntimeTypeSet | null; ensureDraft: () => Promise<Version>; onSnapshotChange: () => Promise<void>; notify: (text: string) => void; onRunAction: (actionId: string, subject: { id: string; labels: string[]; properties: Record<string, unknown> }) => void; fail: (reason: unknown) => void; entityLimit: number }) {
  ...
  return <ResizableManagerGrid storageKey="ontology.manager-split.entities">
    <div className="panel functional-panel result-list">
      ...
      <h2>{rows.length} 条</h2>
      <p className="subtle">默认按类上设置的显示名称搜索；未配置显示属性时按其他属性匹配。</p>
