...
import { Activity, AlertCircle, BookOpen, CheckCircle2, CircleDot, Database, FileCheck2, GitBranch, Link2, Loader2, LogOut, Merge, Network, Pencil, Plus, RefreshCcw, Search, Settings2, ShieldCheck, TableProperties, Trash2, UserRound, X } from "lucide-react";
...
export function FunctionalWorkbench() {
  ...
  return <main className="functional-shell">
    ...
    <section className="functional-content"><header><div><p>图谱治理 / {view}</p><h1>{selectedTarget?.name ?? "连接 Neo4j 目标"}</h1></div><div className="header-state">{selectedTarget ? <><span className="state-dot" />已选择目标</> : "需要登记目标"}</div></header><Notice message={error ?? message} error={Boolean(error)} />
      ...
      {view === "ontology" && <OntologyManager definition={definition} draft={draft} user={userProp} runtimeTypes={runtimeTypes} save={saveDefinition} validate={validate} publish={publish} notify={notify} fail={fail} />}
...
function OntologyManager({ definition, draft, user, runtimeTypes, save, validate, publish, notify, fail }: { definition: Definition; draft: Version | null; user: User; runtimeTypes: RuntimeTypeSet | null; save: (definition: Definition) => Promise<void>; validate: () => Promise<void>; publish: () => Promise<void>; notify: (text: string) => void; fail: (reason: unknown) => void }) {
  ...
  const [importPlan, setImportPlan] = useState<{ newEntities: EntityType[]; newRelations: RelationType[]; mergedRelations: RelationType[]; backfilled: number; unresolved: number } | null>(null);
  ...
  const importFromDatabase = () => {
    ...
    const newEntities: EntityType[] = runtimeTypes.labels.filter((item) => !entityNames.has(item.name)).map((item) => ({ id: crypto.randomUUID(), name: item.name, description: "", displayProperty: "", properties: [] }));
    ...
    const newRelations: RelationType[] = runtimeTypes.relationshipTypes.filter((item) => !definition.relationshipTypes.some((existing) => existing.name === item.name)).map((item) => {
      ...
      return { id: crypto.randomUUID(), name: item.name, sourceEntityTypeId: resolve(endpoint?.source), targetEntityTypeId: resolve(endpoint?.target), properties: [] };
    });
    const mergedRelations = definition.relationshipTypes.map((item) => {
      const endpoint = endpoints[item.name];
      if (!endpoint) return item;
      const sourceEntityTypeId = item.sourceEntityTypeId || resolve(endpoint.source);
      const targetEntityTypeId = item.targetEntityTypeId || resolve(endpoint.target);
      if (sourceEntityTypeId === item.sourceEntityTypeId && targetEntityTypeId === item.targetEntityTypeId) return item;
      return { ...item, sourceEntityTypeId, targetEntityTypeId };
    });
    const backfilled = mergedRelations.filter((item, index) => item.sourceEntityTypeId !== definition.relationshipTypes[index].sourceEntityTypeId || item.targetEntityTypeId !== definition.relationshipTypes[index].targetEntityTypeId).length;
    const unresolved = [...mergedRelations, ...newRelations].filter((item) => !item.sourceEntityTypeId || !item.targetEntityTypeId).length;
    setImportPlan({ newEntities, newRelations, mergedRelations, backfilled, unresolved });
  };
  const mergeImport = async () => {
    const plan = importPlan;
    if (!plan) return;
    setImportPlan(null);
    if (!plan.newEntities.length && !plan.newRelations.length && !plan.backfilled) { notify("数据库中的类型已全部在草稿中，草稿未变化。"); return; }
    try { await save({ ...definition, entityTypes: [...definition.entityTypes, ...plan.newEntities], relationshipTypes: [...plan.mergedRelations, ...plan.newRelations] }); notify("已从图数据库合并类型到草稿。"); } catch (reason) { fail(reason); }
  };
  const overwriteImport = async () => {
    ...
    const endpoints = runtimeTypes.relationshipEndpoints ?? {};
    const resolve = (label: string | undefined) => (label ? entityIdByName.get(label) ?? "" : "");
    const entityTypes: EntityType[] = runtimeTypes.labels.map((item) => ({ id: entityIdByName.get(item.name) ?? "", name: item.name, description: "", displayProperty: "", properties: [] }));
    const relationshipTypes: RelationType[] = runtimeTypes.relationshipTypes.map((item) => {
      const endpoint = endpoints[item.name];
      return { id: crypto.randomUUID(), name: item.name, sourceEntityTypeId: resolve(endpoint?.source), targetEntityTypeId: resolve(endpoint?.target), properties: [] };
    });
    try { await save({ ...definition, entityTypes, relationshipTypes }); notify("已用图数据库类型覆盖草稿。"); } catch (reason) { fail(reason); }
  };
