...
function OntologyManager({ definition, draft, targetId, user, runtimeTypes, refreshRuntimeTypes, save, validate, publish, notify, onOpenActions, fail }: { definition: Definition; draft: Version | null; targetId?: string; user: User; runtimeTypes: RuntimeTypeSet | null; refreshRuntimeTypes: () => void; save: (definition: Definition) => Promise<void>; validate: () => Promise<void>; publish: () => Promise<void>; notify: (text: string) => void; onOpenActions: () => void; fail: (reason: unknown) => void }) {
  ...
  const updateRelation = async (id: string, patch: { name: string; description?: string; sourceEntityTypeId: string; targetEntityTypeId: string; sourceKeyMappings?: KeyMapping[]; targetKeyMappings?: KeyMapping[]; linkSource?: RelationType["linkSource"]; properties: Property[] }) => { try { const next = structuredClone(definition); const idx = next.relationshipTypes.findIndex((item) => item.id === id); if (idx < 0) return; if (next.relationshipTypes.some((item, i) => i !== idx && item.name === patch.name.trim())) throw new Error("关系类型名称已存在。"); next.relationshipTypes[idx] = { ...next.relationshipTypes[idx], name: patch.name.trim(), description: (patch.description ?? "").trim(), sourceEntityTypeId: patch.sourceEntityTypeId, targetEntityTypeId: patch.targetEntityTypeId, sourceKeyMappings: patch.sourceKeyMappings ?? [], targetKeyMappings: patch.targetKeyMappings ?? [], linkSource: patch.linkSource ?? EMPTY_LINK_SOURCE, properties: patch.properties }; await save(next); notify("关系类型及其属性已更新。"); } catch (reason) { fail(reason); } };
