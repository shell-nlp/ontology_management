...
function Overview({ target, draft, published, onNavigate, onOpenOntology }: { target: Target | null; draft: Version | null; published: Version | null; onNavigate: (view: View) => void; onOpenOntology: () => void }) { return <section className="panel functional-panel overview-panel"><span className="eyebrow">本体控制室</span><h2>{target ? "本体与运行时状态" : "开始登记第一个目标"}</h2>{target ? <div className="status-grid"><div><b>{draft ? `v${draft.version_number}` : "无"}</b><span>当前草稿</span></div><div><b>{published ? `v${published.version_number}` : "无"}</b><span>已发布版本</span></div><div><b>{published?.definition.entityTypes.length ?? 0}</b><span>已发布实体类型</span></div><div><b>{published?.definition.relationshipTypes.length ?? 0}</b><span>已发布关系类型</span></div></div> : <p>先在“连接目标”登记 Neo4j URI、数据库、用户名与密码。密码会加密保存。</p>}<div className="functional-actions">{target && <button className="action" onClick={() => onNavigate("graph")}><Network size={16} />打开图谱管理</button>}{target && <button className="action" onClick={() => onNavigate("ontology")}><BookOpen size={16} />配置本体草稿</button>}<button className="action primary" onClick={() => target ? onOpenOntology() : onNavigate("targets")}><span className="arrow">→</span>{target ? "查看本体" : "登记 Neo4j 目标"}</button></div></section>; }
...
function OntologyManager({ definition, draft, user, save, validate, publish, notify, fail }: { definition: Definition; draft: Version | null; user: User; save: (definition: Definition) => Promise<void>; validate: () => Promise<void>; publish: () => Promise<void>; notify: (text: string) => void; fail: (reason: unknown) => void }) {
  ...
  const updateEntity = async (id: string, patch: { name: string; description: string; properties: Property[] }) => { try { const next = structuredClone(definition); const idx = next.entityTypes.findIndex((item) => item.id === id); if (idx < 0) return; if (next.entityTypes.some((item, i) => i !== idx && item.name === patch.name.trim())) throw new Error("实体类型名称已存在。"); next.entityTypes[idx] = { ...next.entityTypes[idx], name: patch.name.trim(), description: patch.description.trim(), properties: patch.properties }; await save(next); notify("实体类型及其属性已更新。"); } catch (reason) { fail(reason); } };
...
function TypeEditDialog({ kind, entity, relation, entityTypes, onClose, onSave }: { kind: "entity" | "relation"; entity?: EntityType | null; relation?: RelationType | null; entityTypes: EntityType[]; onClose: () => void; onSave: (payload: { name: string; description?: string; sourceEntityTypeId?: string; targetEntityTypeId?: string; properties: Property[] }) => Promise<void> }) {
  ...
  const removeProperty = (index: number) => { setProperties((current) => current.filter((_, i) => i !== index)); if (editingProp === index) { setEditingProp(null); setPropDraft(null); } };
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!name.trim()) return; setBusy(true); try { await onSave(kind === "entity" ? { name, description, properties } : { name, sourceEntityTypeId: source, targetEntityTypeId: target, properties }); onClose(); } finally { setBusy(false); } };
  return <div className="dialog-backdrop" role="presentation"><form className="dialog graph-dialog" onSubmit={submit}><button type="button" className="close-button" onClick={onClose} title="关闭"><X size={18} /></button><div className="dialog-icon">{kind === "entity" ? <CircleDot size={22} /> : <Link2 size={22} />}</div><span className="eyebrow">{kind === "entity" ? "实体类型" : "关系契约"}</span><h2>编辑{kind === "entity" ? "实体类型" : "关系类型"}</h2><label>名称<input value={name} onChange={(event) => setName(event.target.value)} required /></label>{kind === "entity" ? <label>说明<input value={description} onChange={(event) => setDescription(event.target.value)} placeholder="业务含义" /></label> : <><label>起始实体类型<select value={source} onChange={(event) => setSource(event.target.value)} required><option value="">选择类型</option>{entityTypes.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>终止实体类型<select value={target} onChange={(event) => setTarget(event.target.value)} required><option value="">选择类型</option>{entityTypes.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label></>}<div className="dialog-divider" /><div className="dialog-section-head"><span className="eyebrow">属性配置</span><h3>{kind === "entity" ? "实体属性" : "关系属性"}</h3></div><p className="dialog-hint">在同一处维护该类型全部属性，保存时一并写入草稿。</p>{properties.length > 0 && <div className="dialog-prop-list">{properties.map((prop, index) => <div className="dialog-prop-row" key={`${prop.name}-${index}`}>{editingProp === index && propDraft ? <><input value={propDraft.name} onChange={(event) => setPropDraft({ ...propDraft, name: event.target.value })} /><select value={propDraft.dataType} onChange={(event) => setPropDraft({ ...propDraft, dataType: event.target.value as Property["dataType"] })}>{typeOptions.map((item) => <option key={item}>{item}</option>)}</select><label className="check-label"><input type="checkbox" checked={propDraft.required} onChange={(event) => setPropDraft({ ...propDraft, required: event.target.checked })} />必填</label><button type="button" className="action compact" onClick={saveProperty}><CheckCircle2 size={13} />保存</button><button type="button" className="action compact" onClick={() => { setEditingProp(null); setPropDraft(null); setLocalError(""); }}>取消</button></> : <><TableProperties size={15} /><b>{prop.name}</b><span>{prop.dataType}</span><em>{prop.required ? "必填" : "可选"}</em><button type="button" className="action compact" onClick={() => startEditProperty(index)}><Pencil size={12} />编辑</button><button type="button" className="action compact danger" onClick={() => removeProperty(index)}><Trash2 size={12} />删除</button></>}</div>)}</div>}<form className="inline-form dialog-prop-add" onSubmit={addProperty}><input value={propName} onChange={(event) => setPropName(event.target.value)} placeholder="新属性名称" /><select value={dataType} onChange={(event) => setDataType(event.target.value as Property["dataType"])}>{typeOptions.map((item) => <option key={item}>{item}</option>)}</select><label className="check-label"><input type="checkbox" checked={required} onChange={(event) => setRequired(event.target.checked)} />必填</label><button className="action primary"><Plus size={14} />添加属性</button></form>{localError && <p className="dialog-error">{localError}</p>}<div className="dialog-actions"><button type="button" className="quiet-button" onClick={onClose}>取消</button><button className="primary-button" disabled={busy}>{busy ? "保存中…" : "保存修改"}</button></div></form></div>;
}
  ...
  return <section className="manager-grid">
    <div className="panel functional-panel result-list">
      ...
      <div className="manager-toolbar">
        <select value={label} onChange={(event) => { setLabel(event.target.value); void load(event.target.value, search); }}><option value="">全部标签</option>{(runtimeTypes?.labels ?? []).map((item) => <option key={item.name} value={item.name}>{item.name}</option>)}</select>
...
function EntityCreateDialog({ published, runtimeTypes, onClose, onCreate }: { published: Version | null; runtimeTypes: RuntimeTypeSet | null; onClose: () => void; onCreate: (label: string, properties: Record<string, unknown>) => Promise<void> }) {
  ...
  const definitions = published?.definition.entityTypes.find((item) => item.name === value)?.properties ?? null;
  ...
  return <div className="dialog-backdrop" role="presentation"><form className="dialog graph-dialog" onSubmit={(event) => { event.preventDefault(); if (!value.trim()) return; setBusy(true); void onCreate(value.trim(), properties).finally(() => setBusy(false)); }}><button type="button" className="close-button" onClick={onClose} title="关闭"><X size={18} /></button><div className="dialog-icon"><CircleDot size={22} /></div><span className="eyebrow">新建实体</span><h2>选择节点标签</h2><label>标签{customMode ? <input autoFocus value={customLabel} onChange={(event) => setCustomLabel(event.target.value)} placeholder="输入新的标签" /> : <select value={label} onChange={(event) => setLabel(event.target.value)} required><option value="">选择标签</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select>}</label><button type="button" className="graph-dialog-custom" onClick={() => setCustomMode((current) => !current)}>{customMode ? "改为从列表选择" : "使用自定义标签"}</button><PropertyEditor definitions={definitions ?? []} values={properties} mode={definitions ? "managed" : "raw"} onChange={setProperties} /><div className="dialog-actions"><button type="button" className="quiet-button" onClick={onClose}>取消</button><button className="primary-button" disabled={!value.trim() || busy}>{busy ? "创建中…" : "创建实体"}</button></div></form></div>;
}
