...
const emptyDefinition: Definition = { groups: [], interfaces: [], entityTypes: [], relationshipTypes: [], actionTypes: [], rules: [] };
  ...
  return (
    <div className="version-bar header-version-bar">
      ...
      <div className="functional-actions">
        {user.role === "ADMIN" && !draft && <button className="action primary" onClick={() => void onCreate().catch(fail)}><Plus size={14} />创建草稿</button>}
...
function TargetManager({ targets, refresh, selectedId, onSelect, onNew, notify, fail }: { targets: Target[]; refresh: () => Promise<void>; selectedId: string; onSelect: (id: string) => void; onNew: () => void; notify: (text: string) => void; fail: (reason: unknown) => void }) {
  ...
  return <section className="stack">
    <div className="panel functional-panel target-action-bar"><div><span className="eyebrow">图引擎配置</span><b>{targets.length} 项可用图引擎</b><p className="subtle">{census ? `按引擎分组：${census}。` : "正在读取图引擎配置。"}内置类型图由平台自带；Jena 连接可按需添加。</p></div><button className="action primary" onClick={onNew}><Plus size={15} />新建 Jena 连接</button></div>
...
function OntologyManager({ definition, draft, targetId, user, runtimeTypes, refreshRuntimeTypes, save, validate, publish, notify, onOpenActions, fail }: { definition: Definition; draft: Version | null; targetId?: string; user: User; runtimeTypes: RuntimeTypeSet | null; refreshRuntimeTypes: () => void; save: (definition: Definition) => Promise<void>; validate: () => Promise<void>; publish: () => Promise<void>; notify: (text: string) => void; onOpenActions: () => void; fail: (reason: unknown) => void }) {
  ...
  // 一级标签：可视化建模 → 概念分组 → 对象类型 → 关系类型 → 接口。
  // 「对象类型 / 关系类型」原先藏在「表单」标签里当二级标签，2026-09-16 提到一级，表单标签删除。
  const [mode, setMode] = useState<"visual" | "groups" | "entities" | "relations" | "interfaces">("visual");
  /** 画布上点接口节点后要「编辑接口」：切到「接口」标签并选中它。 */
  const [interfaceFocusId, setInterfaceFocusId] = useState("");
  /** 建模体检面板开没开。默认收起：它是"回头看一眼"的工具，不该一进来就占掉半屏。 */
  const [reviewOpen, setReviewOpen] = useState(false);
  // 页头按钮上挂一个角标，说清"有几项该改" —— 不点开也知道要不要点开。
  const reviewSummary = useMemo(() => summarizeModelingReview(reviewOntologyModel(definition)), [definition]);
  // 画布上新建时 id 由调用方生成，便于创建后立刻选中刚出现的节点或连线。
  // 概念分组在保存这一刻才定 id：界面上填的是分组名，重名就复用已有的那一条（resolveGroup）。
  const createEntity = async (id: string, payload: EntityPayload) => { try { const trimmed = payload.name.trim(); if (!trimmed) throw new Error("请填写对象类型的名称。"); if (definition.entityTypes.some((item) => item.name === trimmed)) throw new Error("对象类型的名称已存在。"); const named = resolveGroup(definition.groups ?? [], payload.groupName ?? ""); await save({ ...definition, groups: named.groups, entityTypes: [...definition.entityTypes, { id, name: trimmed, description: payload.description.trim(), displayProperty: payload.displayProperty.trim(), groupId: named.groupId, implements: payload.implements ?? [], properties: payload.properties, sources: payload.sources }] }); notify("对象类型已加入草稿。"); } catch (reason) { fail(reason); } };
  const createRelation = async (id: string, payload: RelationPayload) => { try { const trimmed = payload.name.trim(); if (!trimmed) throw new Error("请填写关系类型名称。"); if (!payload.sourceEntityTypeId || !payload.targetEntityTypeId) throw new Error("请选择关系的起始对象类型与终止对象类型。"); if (definition.relationshipTypes.some((item) => item.name === trimmed)) throw new Error("关系类型名称已存在。"); await save({ ...definition, relationshipTypes: [...definition.relationshipTypes, { id, name: trimmed, description: (payload.description ?? "").trim(), sourceEntityTypeId: payload.sourceEntityTypeId, targetEntityTypeId: payload.targetEntityTypeId, sourceKeyMappings: payload.sourceKeyMappings ?? [], targetKeyMappings: payload.targetKeyMappings ?? [], properties: payload.properties }] }); notify("关系类型已加入草稿。"); } catch (reason) { fail(reason); } };
  ...
  const addRelation = async (event: FormEvent) => { event.preventDefault(); try { if (!rel.name || !rel.source || !rel.target) throw new Error("请填写关系类型和两个端点。"); await save({ ...definition, relationshipTypes: [...definition.relationshipTypes, { id: newId(), name: rel.name, sourceEntityTypeId: rel.source, targetEntityTypeId: rel.target, sourceKeyMappings: [], targetKeyMappings: [], properties: [] }] }); setRel({ name: "", source: "", target: "" }); } catch (reason) { fail(reason); } };
  const updateEntity = async (id: string, patch: { name: string; description: string; displayProperty?: string; groupName?: string; implements?: string[]; properties: Property[]; sources?: EntityType["sources"] }) => { try { const next = structuredClone(definition); const idx = next.entityTypes.findIndex((item) => item.id === id); if (idx < 0) return; if (next.entityTypes.some((item, i) => i !== idx && item.name === patch.name.trim())) throw new Error("对象类型的名称已存在。"); // groupName 没传就是"这次不改分组"；传了空串才是"移出分组"。
    const named = patch.groupName === undefined ? null : resolveGroup(next.groups ?? [], patch.groupName); if (named) next.groups = named.groups; next.entityTypes[idx] = { ...next.entityTypes[idx], name: patch.name.trim(), description: patch.description.trim(), displayProperty: (patch.displayProperty ?? "").trim(), groupId: named ? named.groupId : next.entityTypes[idx].groupId, implements: patch.implements ?? next.entityTypes[idx].implements ?? [], properties: patch.properties, sources: patch.sources }; await save(next); notify("对象类型及其属性已更新。"); } catch (reason) { fail(reason); } };
  const updateRelation = async (id: string, patch: { name: string; description?: string; sourceEntityTypeId: string; targetEntityTypeId: string; sourceKeyMappings?: KeyMapping[]; targetKeyMappings?: KeyMapping[]; properties: Property[] }) => { try { const next = structuredClone(definition); const idx = next.relationshipTypes.findIndex((item) => item.id === id); if (idx < 0) return; if (next.relationshipTypes.some((item, i) => i !== idx && item.name === patch.name.trim())) throw new Error("关系类型名称已存在。"); next.relationshipTypes[idx] = { ...next.relationshipTypes[idx], name: patch.name.trim(), description: (patch.description ?? "").trim(), sourceEntityTypeId: patch.sourceEntityTypeId, targetEntityTypeId: patch.targetEntityTypeId, sourceKeyMappings: patch.sourceKeyMappings ?? [], targetKeyMappings: patch.targetKeyMappings ?? [], properties: patch.properties }; await save(next); notify("关系类型及其属性已更新。"); } catch (reason) { fail(reason); } };
  ...
  return <section className="stack"><div className="panel functional-panel"><div className="title-row"><div><span className="eyebrow">本体建模</span><h2>{draft ? `v${draft.version_number} · 未发布` : "尚无草稿"}</h2></div><div className="functional-actions"><button className="action" disabled={user.role !== "ADMIN" || !runtimeTypes} onClick={() => void extractTypesFromSnapshot()} title="从当前版本快照提取对象和关系类型并合并到草稿"><Database size={16} />从快照提取类型</button><button className="action" onClick={refreshRuntimeTypes} title="重新读取当前版本快照中的对象、关系、端点和属性推断"><RefreshCcw size={16} />刷新快照数据</button><button className="action" disabled={user.role !== "ADMIN"} onClick={() => validate().catch(fail)}><FileCheck2 size={16} />校验</button><button className="action" onClick={() => setReviewOpen((current) => !current)} title="给草稿做一次建模体检：空壳 / 孤悬 / 主键列接不上 / 命名打架 …… 只给建议，不挡发布"><Stethoscope size={16} />建模体检{reviewSummary.warn ? <em className="review-badge">{reviewSummary.warn}</em> : null}</button><button className="action primary" disabled={user.role !== "ADMIN"} onClick={() => publish().catch(fail)}><ShieldCheck size={16} />发布</button></div></div>{mode === "visual" ? <p className="subtle">在画布上直接搭模型：加对象类型、从节点拉出关系类型、点开补属性与端点，每次改动都会立刻存进草稿。</p> : mode === "groups" ? <p className="subtle">按业务域把对象类型归堆：分组的颜色就是画布「按逻辑分组」时画的那个框。分组只影响怎么摆、怎么画，不进图库、也不改对象类型的定义。</p> : mode === "entities" ? <p className="subtle">对象类型的清单与新增入口：改动会先保存为草稿，发布前会检查端点契约与必填属性；点单个类型的「编辑」一并配置属性、数据来源与实现的接口。</p> : mode === "relations" ? <p className="subtle">关系类型的清单与新增入口：每条关系类型都连两个对象类型，可以带自己的属性。关系类型是**双向**的：两个方向都能走，不用再建反向的那一条。</p> : <p className="subtle">接口是抽象契约：只描述实现它的对象类型必须有哪些属性、哪些关系，不绑数据、也不能被实例化。应用按接口统一消费，新加一个实现了它的对象类型不用改应用。</p>}</div><div className="view-switcher" aria-label="本体编辑方式"><button className={mode === "visual" ? "active" : ""} aria-pressed={mode === "visual"} onClick={() => setMode("visual")}>可视化建模</button><button className={mode === "groups" ? "active" : ""} aria-pressed={mode === "groups"} onClick={() => setMode("groups")}>概念分组 <b>{definition.groups.length}</b></button><button className={mode === "entities" ? "active" : ""} aria-pressed={mode === "entities"} onClick={() => setMode("entities")}>对象类型 <b>{definition.entityTypes.length}</b></button><button className={mode === "relations" ? "active" : ""} aria-pressed={mode === "relations"} onClick={() => setMode("relations")}>关系类型 <b>{definition.relationshipTypes.length}</b></button><button className={mode === "interfaces" ? "active" : ""} aria-pressed={mode === "interfaces"} onClick={() => setMode("interfaces")}>接口 <b>{definition.interfaces.length}</b></button></div>{reviewOpen ? <ModelingReviewPanel definition={definition} onLocate={(scope) => { if (scope === "OBJECT_TYPE") setMode("entities"); else if (scope === "RELATION_TYPE") setMode("relations"); else if (scope === "INTERFACE") setMode("interfaces"); else if (scope === "GROUP") setMode("groups"); else if (scope === "ACTION" || scope === "RULE") onOpenActions(); }} /> : null}{mode === "interfaces" ? <InterfaceManager definition={definition} canEdit={user.role === "ADMIN"} save={save} notify={notify} fail={fail} focusId={interfaceFocusId} /> : mode === "groups" ? <ConceptGroupManager definition={definition} canEdit={user.role === "ADMIN"} save={save} notify={notify} fail={fail} /> : mode === "visual" ? <OntologyBuilder definition={definition} targetId={targetId} canEdit={user.role === "ADMIN"} hasSnapshot={Boolean(runtimeTypes)} onCreateEntity={createEntity} onUpdateEntity={async (id, payload) => updateEntity(id, { name: payload.name, description: payload.description, displayProperty: payload.displayProperty, groupName: payload.groupName, implements: payload.implements, properties: payload.properties, sources: payload.sources })} onDeleteEntity={deleteEntity} onCreateRelation={createRelation} onUpdateRelation={async (id, payload) => updateRelation(id, { name: payload.name, description: payload.description, sourceEntityTypeId: payload.sourceEntityTypeId, targetEntityTypeId: payload.targetEntityTypeId, properties: payload.properties })} onDeleteRelation={deleteRelation} onSaveDefinition={save} onExtract={extractTypesFromSnapshot} onOpenActions={onOpenActions} onOpenGroups={() => setMode("groups")} onOpenInterface={(id) => { setInterfaceFocusId(id); setMode("interfaces"); }} onNotify={notify} onFail={fail} /> : mode === "entities" ? <div className="manager-grid"><form className="panel functional-panel form-panel" onSubmit={addEntity}><span className="eyebrow">对象类型</span><h2>新增对象类型</h2><label>名称<input value={name} onChange={(event) => setName(event.target.value)} placeholder="例如：客户" required /></label><label>说明<input value={description} onChange={(event) => setDescription(event.target.value)} placeholder="业务含义" /></label><button className="action primary"><Plus size={16} />加入草稿</button></form><div className="panel functional-panel type-list"><span className="eyebrow">对象类型清单</span><h2>{definition.entityTypes.length} 个对象类型</h2>{definition.entityTypes.map((item) => <div className="type-row" key={item.id}><CircleDot size={17} /><b>{item.name}</b><span>{item.description || "未填写说明"}</span><em>{item.properties.length} 属性</em>{item.sources?.length ? <i className="type-source"><Table2 size={11} />{sourceSummary(item)}</i> : null}<button className="action compact" disabled={user.role !== "ADMIN"} onClick={() => setEditingEntity(item)}><Pencil size={13} />编辑</button><button className="action compact danger" disabled={user.role !== "ADMIN"} onClick={() => void deleteEntity(item.id)}><Trash2 size={13} />删除</button></div>)}{!definition.entityTypes.length && <p className="empty">尚未创建对象类型。</p>}</div></div> : <div className="manager-grid"><form className="panel functional-panel form-panel" onSubmit={addRelation}><span className="eyebrow">关系类型</span><h2>新增关系类型</h2><label>关系名称<input value={rel.name} onChange={(event) => setRel({ ...rel, name: event.target.value })} placeholder="例如：负责" required /></label><label>起始对象类型<select value={rel.source} onChange={(event) => setRel({ ...rel, source: event.target.value })} required><option value="">选择类型</option>{definition.entityTypes.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>终止对象类型<select value={rel.target} onChange={(event) => setRel({ ...rel, target: event.target.value })} required><option value="">选择类型</option>{definition.entityTypes.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><button className="action primary"><Plus size={16} />加入草稿</button></form><div className="panel functional-panel type-list"><span className="eyebrow">关系契约清单</span><h2>{definition.relationshipTypes.length} 个关系类型</h2>{definition.relationshipTypes.map((item) => <div className="type-row" key={item.id}><Link2 size={17} /><b>{item.name}</b><span>{item.sourceEntityTypeId ? definition.entityTypes.find((e) => e.id === item.sourceEntityTypeId)?.name ?? "?" : "未指定"} ↔ {item.targetEntityTypeId ? definition.entityTypes.find((e) => e.id === item.targetEntityTypeId)?.name ?? "?" : "未指定"}</span><em>{item.properties.length} 属性</em><button className="action compact" disabled={user.role !== "ADMIN"} onClick={() => setEditingRelation(item)}><Pencil size={13} />编辑</button><button className="action compact danger" disabled={user.role !== "ADMIN"} onClick={() => void deleteRelation(item.id)}><Trash2 size={13} />删除</button></div>)}{!definition.relationshipTypes.length && <p className="empty">尚未创建关系类型。</p>}</div></div>}
      {editingEntity && <TypeEditDialog kind="entity" entity={editingEntity} entityTypes={definition.entityTypes} groups={definition.groups} interfaces={definition.interfaces} onClose={() => setEditingEntity(null)} onSave={async (payload) => { await updateEntity(editingEntity.id, { name: payload.name, description: payload.description ?? "", displayProperty: payload.displayProperty ?? "", groupName: payload.groupName, implements: payload.implements, properties: payload.properties, sources: payload.sources }); setEditingEntity(null); }} />}{editingRelation && <TypeEditDialog kind="relation" relation={editingRelation} entityTypes={definition.entityTypes} interfaces={definition.interfaces} onClose={() => setEditingRelation(null)} onSave={async (payload) => { await updateRelation(editingRelation.id, { name: payload.name, sourceEntityTypeId: payload.sourceEntityTypeId ?? "", targetEntityTypeId: payload.targetEntityTypeId ?? "", sourceKeyMappings: payload.sourceKeyMappings ?? [], targetKeyMappings: payload.targetKeyMappings ?? [], properties: payload.properties }); setEditingRelation(null); }} />}
...
function GraphManager({ target, user, version, draft, runtimeTypes, onSnapshotChange, notify, fail }: { target: Target | null; user: User; version: Version | null; draft: Version | null; runtimeTypes: RuntimeTypeSet | null; onSnapshotChange: () => Promise<void>; notify: (text: string) => void; fail: (reason: unknown) => void }) {
