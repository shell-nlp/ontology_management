...
export function FunctionalWorkbench() {
  ...
  const refreshRuntimeTypes = () => { if (!targetId) return; void api<RuntimeTypeSet>(`/api/instances/types?targetId=${encodeURIComponent(targetId)}`).then(setRuntimeTypes).catch(() => setRuntimeTypes(null)); };
...
function OntologyManager({ definition, draft, user, runtimeTypes, refreshRuntimeTypes, save, validate, publish, notify, fail }: { definition: Definition; draft: Version | null; user: User; runtimeTypes: RuntimeTypeSet | null; refreshRuntimeTypes: () => void; save: (definition: Definition) => Promise<void>; validate: () => Promise<void>; publish: () => Promise<void>; notify: (text: string) => void; fail: (reason: unknown) => void }) {
  ...
  const mergeImport = async () => {
    ...
    if (!plan) return;
    setImportPlan(null);
    if (!plan.newEntities.length && !plan.newRelations.length && !plan.changedEntities && !plan.changedRelations) { notify("数据库中的类型已全部在草稿中，草稿未变化。"); return; }
    try { await save({ ...definition, entityTypes: [...plan.mergedEntities, ...plan.newEntities], relationshipTypes: [...plan.mergedRelations, ...plan.newRelations] }); notify("已从图数据库合并类型到草稿。"); } catch (reason) { fail(reason); }
  };
