...
function OntologyManager({ definition, draft, targetId, user, runtimeTypes, refreshRuntimeTypes, save, validate, publish, notify, onOpenActions, fail }: { definition: Definition; draft: Version | null; targetId?: string; user: User; runtimeTypes: RuntimeTypeSet | null; refreshRuntimeTypes: () => void; save: (definition: Definition) => Promise<void>; validate: () => Promise<void>; publish: () => Promise<void>; notify: (text: string) => void; onOpenActions: () => void; fail: (reason: unknown) => void }) {
  ...
  const createEntity = async (id: string, payload: EntityPayload) => { try { const trimmed = payload.name.trim(); if (!trimmed) throw new Error("请填写对象类型的名称。"); if (definition.entityTypes.some((item) => item.name === trimmed)) throw new Error("对象类型的名称已存在。"); await save({ ...definition, entityTypes: [...definition.entityTypes, { id, name: trimmed, description: payload.description.trim(), displayProperty: payload.displayProperty.trim(), parents: payload.parents, properties: payload.properties, sources: payload.sources }] }); notify("对象类型已加入草稿。"); } catch (reason) { fail(reason); } };
  ...
  const updateEntity = async (id: string, patch: { name: string; description: string; displayProperty?: string; parents?: string[]; properties: Property[]; sources?: EntityType["sources"] }) => { try { const next = structuredClone(definition); const idx = next.entityTypes.findIndex((item) => item.id === id); if (idx < 0) return; if (next.entityTypes.some((item, i) => i !== idx && item.name === patch.name.trim())) throw new Error("对象类型的名称已存在。"); next.entityTypes[idx] = { ...next.entityTypes[idx], name: patch.name.trim(), description: patch.description.trim(), displayProperty: (patch.displayProperty ?? "").trim(), parents: patch.parents ?? next.entityTypes[idx].parents ?? [], properties: patch.properties, sources: patch.sources }; await save(next); notify("对象类型及其属性已更新。"); } catch (reason) { fail(reason); } };
...
function GraphManager({ target, user, version, draft, runtimeTypes, mode, onModeChange, onSnapshotChange, notify, fail }: { target: Target | null; user: User; version: Version | null; draft: Version | null; runtimeTypes: RuntimeTypeSet | null; mode: "instances" | "ontology"; onModeChange: (mode: "instances" | "ontology") => void; onSnapshotChange: () => Promise<void>; notify: (text: string) => void; fail: (reason: unknown) => void }) {
  ...
  return <section className="stack">
    ...
    {mode === "instances" ? <>
      ...
      <div className="panel functional-panel graph-head-panel"><div className="title-row"><div><span className="eyebrow">本体骨架</span><h2>查看当前 {graphNoun(target)} 运行结构</h2><p className="subtle">骨架从已发布生效的数据推导：Apache Jena 侧读取实例的 <code>rdf:type</code> 与对象属性，再叠上草稿里声明的类与 <code>rdfs:subClassOf</code>。草稿类型和实例要到发布后才会出现在这里。</p></div><button className="action" disabled={loading} onClick={() => void loadOntology()}><Network size={15} />{loading ? "加载中…" : "刷新本体骨架"}</button></div></div>
