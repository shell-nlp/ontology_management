...
function OntologyManager({ definition, draft, targetId, user, runtimeTypes, refreshRuntimeTypes, save, validate, publish, notify, fail }: { definition: Definition; draft: Version | null; targetId?: string; user: User; runtimeTypes: RuntimeTypeSet | null; refreshRuntimeTypes: () => void; save: (definition: Definition) => Promise<void>; validate: () => Promise<void>; publish: () => Promise<void>; notify: (text: string) => void; fail: (reason: unknown) => void }) {
  ...
  const createEntity = async (id: string, payload: EntityPayload) => { try { const trimmed = payload.name.trim(); if (!trimmed) throw new Error("请填写对象类型名称。"); if (definition.entityTypes.some((item) => item.name === trimmed)) throw new Error("对象类型名称已存在。"); await save({ ...definition, entityTypes: [...definition.entityTypes, { id, name: trimmed, description: payload.description.trim(), displayProperty: payload.displayProperty.trim(), properties: payload.properties }] }); notify("对象类型已加入草稿。"); } catch (reason) { fail(reason); } };
  const createRelation = async (id: string, payload: RelationPayload) => { try { const trimmed = payload.name.trim(); if (!trimmed) throw new Error("请填写关系类型名称。"); if (!payload.sourceEntityTypeId || !payload.targetEntityTypeId) throw new Error("请选择起始与终止对象类型。"); if (definition.relationshipTypes.some((item) => item.name === trimmed)) throw new Error("关系类型名称已存在。"); await save({ ...definition, relationshipTypes: [...definition.relationshipTypes, { id, name: trimmed, sourceEntityTypeId: payload.sourceEntityTypeId, targetEntityTypeId: payload.targetEntityTypeId, properties: payload.properties }] }); notify("关系类型已加入草稿。"); } catch (reason) { fail(reason); } };
