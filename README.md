...
| | 是什么 | 平台里的位置 |
| --- | --- | --- |
| **语义模型** | 本体「是什么」：对象类型（Object Type）与它们的属性、关系类型 | 本体草稿 |
| **本体实例** | 本体里"实际有什么"：对象与关系，以及看它们的图谱（实例图谱 / 本体骨架） | 图谱 · 对象 · 关系 |
...
| 🪪 | **接口（Interfaces）** | 抽象契约：只描述"实现我的对象类型必须有哪些属性与关系"，不绑数据、不能实例化；一个对象类型可实现多个接口、接口可继承接口。图谱「查看本体」里实现边与接口继承边分开画 |
...
内置类型图由代码作为虚拟存储选项提供，不在 `graph_targets` 中创建根资源记录；不需要配置凭据，也不能删除该选项。新建或导入具体本体时才持久化独立的受管目标。在「本体」中新建或导入本体时默认选择它；需要外部服务时仍可新建 Jena 连接。**存储资源本身不是本体**，不会自动生成空本体。类型图只包含定义；为了兼容现有手工对象页面，当前发布视图可以暂存少量实例，但**海量业务对象不能进入这份 JSONB 视图或进程内 RDF Store**。Jena 保留且继续可选，现有 Jena 本体不会自动改变后端；若要迁移定义，可导出本体包，在新存储上导入并核验。当前本体包不携带实例数据。
